...
This file contains all the functions used in Code analysis the main one is extarct()
        which takes a VBA macro source code and returns 54 features using the other functions 
        like concatenate(), words() and Variables()... 
This file also contains the tokenize() function which takes the corpus of documents
        and returns a matrix of tokens used for bag of words feature extraction
This file also contains the TFIDF() function which takes the corpus of documents
        and returns a matrix of TFIDF values used for TFIDF feature  extraction

All these functions are used on the datset provided by Acronis and are run in Extraction.py 
...
def TFIDF(fo):
    ...
    tfIdfVectorizer = TfidfVectorizer(
        stop_words=my_stop_words, max_features=2795, use_idf=True)
    tfIdf = tfIdfVectorizer.fit_transform(fo)
